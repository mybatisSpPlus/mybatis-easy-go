...
public class StepGenerator {
    ...
    private void OverWindowToStep(OverWindow function) throws Exception {
        functionToStep(function.getWindowFunction());
        steps.add(new Step("OVER"));
        steps.add(new Step(function.getWindowAlias()));
    }

    private void OverToStep(Over function) throws Exception {
        functionToStep(function.getWindowFunction());
        steps.add(new Step("OVER("));
    ...
    private void RowNumberToStep() {
        steps.add(new Step("ROW_NUMBER()"));
    }

    private void RankToStep() {
        steps.add(new Step("RANK()"));
    }

    private void PercentRankToStep() {
        steps.add(new Step("PERCENT_RANK()"));
    }

    private void NthValueToStep(NthValue function) throws Exception {
    ...
    private void NtileToStep(Ntile function) {
        steps.add(new Step("NTILE("));
        steps.add(new Step().setStepValue(function.getGroupCount()));
        steps.add(new Step(")"));
    }

    private void LeadToStep(Lead function) throws Exception {
    ...
    private void LastValueToStep(LastValue function) throws Exception {
        steps.add(new Step("LAST_VALUE("));
        fieldToStep(function.getField());
        steps.add(new Step(")"));
    }

    private void LagToStep(Lag function) throws Exception {
    ...
    private void FirstValueToStep(FirstValue function) throws Exception {
        steps.add(new Step("FIRST_VALUE("));
        fieldToStep(function.getField());
        steps.add(new Step(")"));
    }

    private void DenseRankToStep() {
        steps.add(new Step("DENSE_RANK()"));
    }

    private void CumeDistToStep() {
